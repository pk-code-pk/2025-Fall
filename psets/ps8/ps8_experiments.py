...
def run_exhaustive(g):
    return exhaustive_search_coloring(g)

def run_iset_bfs(g):
    return iset_bfs_3_coloring(g)

def run_sat(g):
    return sat_3_coloring(g)
...
def benchmark():
    ...
    algs = [("Exhaustive Coloring", run_exhaustive),
            ("ISET BFS Coloring", run_iset_bfs), 
            ("SAT Coloring", run_sat)]
    ...
    for r in [3,4,5]:
        ...
        for rings in range(subgraph_line_parameter_range[0], subgraph_line_parameter_range[1], subgraph_line_parameter_range[2]):
            ...
            for (alg_name, alg) in algs:
                timedout = False
                try:
                    timed_alg = timeout(seconds=TIMEOUT_LENGTH)(alg)
                    timed_alg(g.clone())
                except TimeoutError:
                    timedout = True
