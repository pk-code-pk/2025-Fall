...
import random
random.seed(120)
from ps8_helpers import (
    timeout, color, generate_line_of_ring_subgraphs, generate_random_linked_cluster, 
    validate_graph_coloring, generate_line_of_complete_subgraphs, generate_complete_graph, 
    load_dimacs_graph, generate_random_graph_gamma, generate_hard_instance_from_coloring
)
from time import time
from ps8 import Graph, sat_3_coloring
    ...
    for label, test_results in tests.items():
        ...
        for i, result in enumerate(test_results):
            ...
            if timedout:
                ...
                print(
                    symbol + " " + result["label"] + " (n = {}, m = {})".format(input.N, sum([len(v_lst) for v_lst in input.edges]) // 2)  \
                    + ": ", "Passed" if con else "Failed", "in {:.2f} seconds".format(diff), end=""
                )
...
testcases = {
    ...
    "cmu-hard-graphs": [
        lambda: load_dimacs_graph(Graph, "le450_25a.txt"),
        lambda: load_dimacs_graph(Graph, "le450_25d.txt"),
        lambda: load_dimacs_graph(Graph, "le1000_25a.txt")
    ],
    "large-random-graphs": [
        lambda: generate_random_graph_gamma(Graph, 1_000, gamma, seed=int(gamma * 10))
        for gamma in [3.3, 3.32, 3.34, 3.36, 3.38, 3.4]
    ],
    "larger-random-graphs": [
        lambda: generate_random_graph_gamma(Graph, 10_000, gamma, seed=int(gamma * 10))
        for gamma in [3.3, 3.32, 3.34, 3.36, 3.38, 3.4]
    ],
    "large-random-graphs-part2": [
        lambda: generate_random_graph_gamma(Graph, 1_000, gamma, seed=int(gamma * 10))
        for gamma in [4.5, 4.52, 4.54, 4.56, 4.58, 4.6]
    ], 
    "large-random-graphs-colored": [
...
def test_3_coloring():
    ...
    tests["SAT 3-Coloring"] += [
        generate_test(
            "Should not work on large 25-colorable graphs",
            g,
            lambda g: validate_graph_coloring(g, sat_3_coloring(g)),
            lambda g: False,
        ) for g in testcases["cmu-hard-graphs"]
    ]
    tests["SAT 3-Coloring"] += [
        generate_test(
            "Should work on large random graphs",
            g,
            lambda g: validate_graph_coloring(g, sat_3_coloring(g)),
            lambda g: True,
        ) for g in testcases["large-random-graphs"]
    ]
    tests["SAT 3-Coloring"] += [
        generate_test(
            "Should work on larger random graphs",
            g,
            lambda g: validate_graph_coloring(g, sat_3_coloring(g)),
            lambda g: True,
        ) for g in testcases["larger-random-graphs"]
    ]
    tests["SAT 3-Coloring"] += [
        generate_test(
            "Should work on large random graphs (part 2)",
            g,
            lambda g: validate_graph_coloring(g, sat_3_coloring(g)),
            lambda g: True,
        ) for g in testcases["large-random-graphs-part2"]
    ]
    tests["SAT 3-Coloring"] += [
        generate_test(
